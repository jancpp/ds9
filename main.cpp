...
template <class T>
void timeDeleteminNotDeletemin (int m, int size, int seed, T *table, double &timeDeletemin, double &timeNotDeletemin) {
    clock_t start = 0;
    int randomNumber = 0;
    srand(seed);
    for (int i=0; i<size; i++) {
        ...
        ////            std::cout << "\n " << timeDeletemin;
        //        } else {
        //            timeNotDeletemin += (clock()-start)/(double)(CLOCKS_PER_SEC/1000);
        //            std::cout << "\n " << timeNotDeletemin;
        //        }
    }
}
...
int main(int argc, char **argv) {
    ...
    std::cout << "... takes a while to genetate results\n\n";
     ...
     tableMax = new MaxHeap();

    // timeForHashTable[seed][0-buildTime, 1-DeleteminTime, 2-notDeleteminTime][mult][run5times]
    double timesBST[5][3][5][5] = {};
    double timesMin[5][3][5][5] = {};
    double timesMax[5][3][5][5] = {};

    // timeAccumulatedUsingDifferentSeeds[0-buildTime, 1-DeleteminTime, 2-notDeleteminTime][mult][run5times]
    double accSeedsBST[3][5][5] = {};
    double accSeedsMin[3][5][5] = {};
    double accSeedsMax[3][5][5] = {};

    // timeAverageUsingDifferentSeeds[0-buildTime, 1-DeleteminTime, 2-notDeleteminTime][mult][5times]
    double aveSeedsBST[3][5][5] = {};
    double aveSeedsMin[3][5][5] = {};
    double aveSeedsMax[3][5][5] = {};

    // timeRunAll5times[0-buildTime, 1-DeleteminTime, 2-notDeleteminTime][sizeMultiplier]
    double acc5timesBST[3][5] = {};
    double acc5timesMin[3][5] = {};
    double acc5timesMax[3][5] = {};

    // timeAverageUsingDifferentDataSizes[0-buildTime, 1-DeleteminTime, 2-notDeleteminTime][sizeMultiplier]
...
//                // HT with BSTaining
//                timeDeleteminNotDeletemin(m, size, seed, tableBST, timesBST[seed][1][mult][times], timesBST[seed][2][mult][times]);
//                // HT with quadratic probing
//                timeDeleteminNotDeletemin(m, size, seed, tableMin, timesMin[seed][1][mult][times], timesMin[seed][2][mult][times]);
//                // HT with double hashing
//                timeDeleteminNotDeletemin(m, size, seed, tableMax, timesMax[seed][1][mult][times], timesMax[seed][2][mult][times]);
//
//                // Accumulate times for diffrent seeds
